...
const paper_map = struct {
    ...
    pub fn neightbours(self: *@This(), _x: usize, _y: usize) !usize {
        ...
        const idx_real = try self.index(x, y);
        self.out_buffer[idx_real] = self.buffer[idx_real];
        for (y_idx) |real_y| {
            for (x_idx) |real_x| {
...
pub fn print_map(buffer: []u8, width: usize) void {
    for (1.., buffer) |i, value| {
...
pub fn main() !void {
    ...
    try array2.appendSlice(alloc, array.items);

    var map: paper_map = .init(array.items, array2.items, width, height);

    var sum: isize = 0;
    var prev_sum: isize = -1;
    while (sum != prev_sum) {
        prev_sum = sum;
        for (0..height) |y| {
            for (0..width) |x| {
                if (try map.value(x, y) != '@') continue;
                const neightbours = try map.neightbours(x, y);
                if (neightbours < 4) {
                    sum += 1;
                }
            }
        ...
        std.debug.print("sum: {}\n", .{sum});
        // print_map(array.items, width);
        // print_map(array2.items, width);
        std.mem.copyForwards(u8, array.items, array2.items);
    }
    std.debug.print("sum: {}\n", .{sum});
}
